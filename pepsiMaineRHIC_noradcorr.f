...
C     Take date as the SEED for the random number generation
      ...
      elseif ((PARL(1).eq.2).and.(PARL(2).eq.1)) then
         massp=1.875612793
      elseif ((PARL(1).eq.3).and.(PARL(2).eq.2)) then
         massp=3.0160293  
      else
        write(*,*) "Currently only p,n,d,he-3 as target supported"
      endif
...
C PYPAR21 must be set after LINIT which sets it to PARP(1)
      if (UsePYPAR21) then
         PYPAR(21)=pypar21val
      endif
